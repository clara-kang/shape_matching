...
class OptVerts:
    ...
    def createVars(self):
        ...
        self.abs_diff_x = self.m.addVars(len(self.vert_locs), name="abs_diff_x")
        self.abs_diff_y = self.m.addVars(len(self.vert_locs), name="abs_diff_y")
        self.t_x = self.m.addVars(len(self.shape_verts), lb = -GRB.INFINITY, name="t_diff_x")
        self.t_y = self.m.addVars(len(self.shape_verts), lb = -GRB.INFINITY, name="t_diff_y")
    ...
    def createConstraints(self):
        for i in range(len(self.shape_verts)):
            for vid in self.shape_verts[i]:
                self.m.addConstr(self.diff_x[vid] + self.t_x[i] == self.px[vid] - self.vert_locs[vid][0])
                self.m.addConstr(self.diff_y[vid] + self.t_y[i] == self.py[vid] - self.vert_locs[vid][1])
    ...
    def addObjective(self):
        ...
        self.m.setObjective(sum(self.abs_diff_x[i] for i in range(len(self.abs_diff_x))) + \
            sum(self.abs_diff_y[i] for i in range(len(self.abs_diff_y))), GRB.MINIMIZE)
