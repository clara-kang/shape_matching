...
def write_pts_out(bndry_verts, file_name):
...
write_pts_out(bndry_verts, "bndry_pts.txt")
# print("shape_curve_ids: ", shape_curve_ids)
# bpy.ops.wm.save_as_mainfile(filepath=bpy.data.filepath)
# ================

import get_polygons_1
# import opt_angles_2
import subprocess
import numpy as np

polygon = get_polygons_1.Polygons(bndry_verts, shape_curve_ids, path_pairs)
polygon.getPolyVerts()
polygon.writeVertsOut("poly_verts.txt")

# subprocess.call(['C:\\gurobi902\\win64\\python37\\bin\\python.exe', 'opt_angles_2.py'])
#
# polygon.readInAngles()
...
for time in range(1):
    ...
    subprocess.call(['C:\\gurobi902\\win64\\python37\\bin\\python.exe', 'opt_angles_3.py'])
    polygon.readInAngles()
    polygon.getShapeRotation()
    polygon.rotateShapes()
    polygon.getPolyVecs()
    ...
    # get objective value
    f = open("opt_verts_3_obj.txt", "r")
    obj_val = float(f.readlines()[0])
    ...
    if obj_val < best_objective:
        ...
        polygon.readInVerts()
        best_verts = polygon.optimized_verts
        best_it = time
...
# polygon.optimized_verts = get_polygons_1.flatten_arr(polygon.poly_verts)
# polygon.intrpCurve()
#=========
cnt = 0
for pid in pids:
    ...
    j = start_vid
    while True:
        setVertUV(pid, j, polygon.bndry_verts_uv[cnt])
        # setVertUV(pid, j, [0, 0])
        # print("set uv ", pid, ", ", j, ", ", polygon.bndry_verts_uv[cnt])
        cnt += 1
        j = (j + 1) % len(patch_ordr_verts[pid])
        if j == start_vid:
            break
